...
echo
actual_out=$("$GRABCHARS" select "$OPTS" 2>/dev/tty)
actual_exit=$?
echo
check_output "$actual_out" "banana" && check_exit "$actual_exit" "1" && pass || fail "expected 'banana' with exit 1"
...
actual_exit=$?
echo
check_output "$actual_out" "apple" && check_exit "$actual_exit" "0" && pass || fail "expected 'apple' (first item)"
...
actual_out=$("$GRABCHARS" select -t3 -dcherry "$OPTS" 2>/dev/tty)
actual_exit=$?
echo
echo    "  Output was: \"$actual_out\""
check_output "$actual_out" "cherry" && check_exit "$actual_exit" "2" && pass || fail "expected 'cherry' with exit 2"

# ─────────────────────────────────────────────────────────────────────────────
test_start "select: -U uppercases filter input"
echo    "  -U converts your typed filter characters to uppercase."
echo    "  Options: YES,NO,MAYBE. Type 'n' — it becomes 'N' and matches 'NO'."
instruct "Type 'n' to filter to 'NO', press Enter"
show_command "select -U YES,NO,MAYBE"
echo
actual_out=$("$GRABCHARS" select -U "YES,NO,MAYBE" 2>/dev/tty)
actual_exit=$?
echo
...
echo
actual_out=$("$GRABCHARS" select --file "$TMPFILE" 2>/dev/tty)
actual_exit=$?
rm -f "$TMPFILE"
echo
check_output "$actual_out" "green" && check_exit "$actual_exit" "1" && pass || fail "expected 'green' with exit 1"
...
echo
actual_out=$("$GRABCHARS" select -Fp "$CITY_OPTS" 2>/dev/tty)
actual_exit=$?
echo
...
echo
actual_out=$("$GRABCHARS" select -Ff "$CITY_OPTS" 2>/dev/tty)
actual_exit=$?
echo
...
echo
actual_out=$("$GRABCHARS" select -Ff "$NEW_OPTS" 2>/dev/tty)
actual_exit=$?
echo
...
echo
actual_out=$("$GRABCHARS" select -Fc "$NEW_OPTS" 2>/dev/tty)
actual_exit=$?
echo
check_output "$actual_out" "new york" && check_exit "$actual_exit" "1" && pass || fail "expected 'new york' with exit 1"
