#!/bin/bash
# forms/spawn-intake.sh — Spawn a multi-field intake form using grabchars.
#
# Usage:
...
# The runner uses absolute paths for grabchars and jq — no PATH needed.

{
    printf '#!/bin/zsh -f\n'
    printf '# Auto-generated by spawn-intake.sh — do not edit\n\n'
    printf 'GRABCHARS=%s\n' "$(printf '%q' "$GRABCHARS")"
    printf 'JQ=%s\n'        "$(printf '%q' "$JQ")"
    ...
    for (( i=0; i<FIELD_COUNT; i++ )); do
        ...
        case "$type" in
            ...
            yn)
                ...
                ;;
            *)
                printf '# Unknown field type "%s" for field %s — skipping\n' "$type" "$name"
                printf 'V_%s=""\n' "$name"
                ;;
        esac
...
# Poll — no hard timeout since form length is variable; cap at 10 minutes
MAX_POLLS=$(( 10 * 60 * 5 ))
elapsed=0
